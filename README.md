...
Agent 系统的失败往往不是单纯的“最终答案不好”，而是可能出在工具选择、参数、执行超时、重试、fallback 或工具成功后的最终回答组织。DeepEval 原有 `ToolCorrectnessMetric` 能评估工具选择和参数正确性，但不覆盖 runtime reliability 和失败阶段归因。本项目补充 deterministic 评测 pipeline，用可复现指标和 benchmark 报告量化 AgentScheduler-Swarm 接入前后的可靠性改善。
...
运行 50 组 deterministic mock benchmark 并生成 `examples/agent_tool_reliability/report.md`：
